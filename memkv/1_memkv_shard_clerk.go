...
func (ck *KVShardClerk) PutAndBroadcast(key uint64, value []byte) ErrorType {
	args := new(PutRequest)
	args.CID = ck.cid
	args.Seq = ck.seq
	args.Key = key
	...
	rawRep := new([]byte)
	ck.c.CallAtLeastOnce(ck.host, KV_PUT_AND_BROADCAST, EncodePutRequest(args), rawRep, 100/*ms*/)
	rep := DecodePutReply(*rawRep)
	return rep.Err
}
